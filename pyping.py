...
class Ping(object):
    ...
    def run_once(self, seq=0, count=None, deadline=None):
        """
        send and receive pings in a loop. Stop if count or until deadline.
        """
        if not self.quiet_output:
...
def ping_once(hostname, timeout=1000, seq=0, packet_size=55, *args, **kwargs):
    p = Ping(hostname, timeout, packet_size, *args, **kwargs)
    return p.run_once(seq)
